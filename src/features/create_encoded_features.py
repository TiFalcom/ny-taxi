...
def main(config_file, dataset_prefix, encoder_type):
    ...
    for table in ['valid', 'test', 'train']:
        ...
        df = pd.read_parquet(os.path.join('data', 'processed', f'{dataset_prefix}_{table}.parquet.gzip'))
        logger.info(f'Shape {dataset_prefix} {table}: {df.shape}')

        # TODO: make a step to keep features configured on config_file
        # This will fix the last script
        ...
        df.to_parquet(
            os.path.join('data', 'processed_encoded', f'{dataset_prefix}_{table}.parquet.gzip'),
            compression='gzip',
            index=False
        )
        logger.info('Saved!')
