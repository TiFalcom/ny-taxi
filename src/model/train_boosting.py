...
def main(config_file, folder_dataset_prefix, hyperparams_file, model_suffix):
    ...
    params = yaml.safe_load(open(os.path.join('src', 'model', 'config', f'{hyperparams_file}.yml'), 'r'))['boosting']

    logger.info(f'LightGBM will be fitted with the following hyperparams: {params}')
    ...
    pickle.dump(model, open(os.path.join('models', 'models', f'lgbm_{model_suffix}.pkl'), 'wb'))
