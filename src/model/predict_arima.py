...
def main(config_file, folder_dataset_prefix, hyperparams_file, model_suffix):
    ...
    params = yaml.safe_load(open(os.path.join('src', 'model', 'config', f'{hyperparams_file}.yml'), 'r'))['arima']

    logger.info(f'ARIMA will be fitted with the following hyperparams: {params}')
    ...
    for location in tqdm([163, 79]):#, 100, 140, 107, 186, 48, 132]):
        ...
        for i in range(744, df.shape[0]+1):
            arima = ARIMA(df.iloc[i-24:i], order=tuple(params['params']['order']))#, freq='h')
            arima = arima.fit()
            forecasts += list(arima.forecast(steps=1).values)

        locations_forecasts[location] = forecasts

    df_forecast = pd.DataFrame(locations_forecasts)

    df_forecast.to_parquet('data/forecast/top8_arima.parquet.gzip', compression='gzip', index=False)

    pickle.dump(locations_forecasts, open('models/models/arima.pkl', 'wb'))
    ...
    print(
        get_metrics(
            [
                (model, ['year_month_day_hour', 'PULocationID'], 'arima top8', df_valid, df_valid)
            ],
            df_valid,
            df_valid,
            target
    ...
    pickle.dump(model, open(os.path.join('models', 'models', f'arima_{model_suffix}.pkl'), 'wb'))
